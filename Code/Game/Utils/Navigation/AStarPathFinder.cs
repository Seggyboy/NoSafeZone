...
public class AStarPathFinder
{
	...
	public IEnumerable<Tile> GetNeighbors( Tile tile )
	{
		...
		for ( int dx = -1; dx <= 1; dx++ )
			for ( int dy = -1; dy <= 1; dy++ )
			{
				...
				if ( !board.IsValidTile( nx, ny ) ) continue;

				var neighbor = tiles[nx, ny];
				if ( neighbor == null ) continue;

				// ✅ Disallow diagonal if cardinal directions are blocked
				if ( dx != 0 && dy != 0 )
				{
					var tileX = tiles[x + dx, y];
					var tileY = tiles[x, y + dy];

					if ( tileX == null || tileY == null ) continue;

					if ( TileUtils.IsBlockedByWall( tile, tileX ) || TileUtils.IsBlockedByWall( tile, tileY ) )
						continue;
				}

				// ✅ Final wall check to neighbor
				if ( TileUtils.IsBlockedByWall( tile, neighbor ) ) continue;
	...
	private bool IsWalkable(Tile tile, bool UseReserveSystem = false)
	{
		...
		if ( tile.HasFlag( TileFlags.Occupied ) ) return false;
