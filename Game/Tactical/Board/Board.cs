...
public sealed class Board : Component
{
	...
	[Property] public int Seed = 0;
	[Property] public GameObject WorldContainer { get; set; } // Just a gameobject to set a parent to for better org.
	[Property] public GameObject WallContainer { get; set; } // Container for walls, so we can easily clear them later if needed.

	[Property] public GameObject DetailContainer { get; set; } // Container for detail objects like furniture and cars.
	[Property] public GameObject FoliageContainer { get; set; } // Container for foliage objects like trees and bushes.
	...
	protected override void OnEnabled()
	{
		...
		DetailContainer = new GameObject
		{
			Name = "DetailContainer",
			WorldPosition = Vector3.Zero,

		};
		FoliageContainer = new GameObject
		{
			Name = "FoliageContainer",
			WorldPosition = Vector3.Zero,
		...
		if ( layer == 0 )
		{
			for ( int x = 0; x < Width; x++ )
			{
				for ( int y = 0; y < Length; y++ )
				{
					...
					tileObj.Parent = WorldContainer; // Fix organization!
	...
	public void GenerateWall(Tile tile)
	{
		...
		wallObj.Parent = WallContainer; // Set parent to the wall container for better organization
		...
		List<string> foliageModels = new List<string>
	{
		...
		for ( int x = 0; x < Width; x++ )
		{
			for ( int y = 0; y < Length; y++ )
			{
				...
				foliage.Parent = FoliageContainer; // Organize foliage under a specific container
