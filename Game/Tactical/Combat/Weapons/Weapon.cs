...
public sealed class Weapon : Component
{
	...
	public void Shoot(Tile OriginTile, Tile DestTile, int Aim, int Panic)
	{
		...
		bullet.WorldScale = new Vector3( 0.5f, 0.5f, 0.5f ); // Set bullet size
		bullet.StartPos = GameObject.WorldPosition + Vector3.Forward * 5f; // Should come from muzzel
	...
	public Tile ChooseTargetTile( Tile targetTile, int Aim, int Panic, Board board )
	{
		...
		for ( int i = 0; i < 10; i++ ) // up to 10 tries to find a valid tile
		{
			...
			Tile offsetTile = board.GetTile( targetTile.X + dx, targetTile.Y + dy, targetTile.Z );
